...
export class DeleteParticipantHandler
  implements RequestHandler<DeleteParticipantCommand, BlogData>
{
  constructor(
  ...
  async handle(command: DeleteParticipantCommand): Promise<BlogData> {
    const { request } = command;
    const { params, existing } = request;
    const { blogId } = params;
    const { id } = existing;

    this.logger.info({ id }, `Deleting participant: ${id}`);

    const args = toDeleteParticipantArgs(request);
